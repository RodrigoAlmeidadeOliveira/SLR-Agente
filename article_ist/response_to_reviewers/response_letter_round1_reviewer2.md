...
Third, [an independent human double-screening pass, blind to the LLM's decisions, is in progress on a stratified sample of the title/abstract and full-text screening (n = 468 T/A, n = 177 FT, 20% of the working set and FT queue respectively) — pending completion at the time of this response / completed, reported in Section [X]]. We report Recall, Lost Evidence (1 − Recall), and its uncertainty (Section [X], Table [X]) [once the human pass concludes: Recall = [VALUE] (95% CI [LOW–HIGH]), Lost Evidence = [VALUE]].
