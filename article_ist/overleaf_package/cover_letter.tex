...
\address{Rodrigo Almeida de Oliveira, M.Sc. \\ Pontif\'icia Universidade Cat\'olica do Paran\'a \\ Programa de P\'os-Gradua\c c\~ao em Inform\'atica (PPGIa) \\ Curitiba, PR, Brazil \\ rodrigo.almeida1@pucpr.edu.br}
...
This manuscript reports a systematic mapping study at the
...
\item \textbf{Methodological:} we operationalise an LLM-assisted screening
      ...
      $0.694$ for full-text) on a stratified random 20\% LLM--LLM sample.
      Independent human double-screening ($n=468$ T/A, $n=177$ FT) showed
      fair/slight agreement and is reported as a limitation. The protocol
      extends the recent work of Khraisha et~al.~(2024) and
      Syriani et~al.~(2023) to the software-process-mining domain at a
      scale of 6{,}147 papers screened across two corpus tiers (5{,}783
      after cross-source deduplication plus 364 control and manually
      imported items).
\item \textbf{Empirical:} we confirm 340 distinct primary studies after
      cross-tier deduplication (404 raw confirmations: 169 working-set
      + 212 + 23 auxiliary) covering 1995--2026. All 169 working-set
      studies were scored against an 8-criterion reporting-quality rubric
      (Dyb{\aa} and Dings{\o}yr, 2008; 136 with QA $\geq 4/8$). Combined-tier
      findings use the 318-study analytical subset (259 with QA $\geq 4/8$).
\item \textbf{Synthetic:} we report five empirically grounded findings
      ...
      four-direction research agenda (RA1--RA4). Only 1 of 340 papers
      jointly satisfies TF-PM $\cap$ TF-ST $\cap$ TF-FC --- evidence that
      the mapped gap is structural rather than an artefact of the search
      queries. PATHCAST is sketched as a future-work hypothesis, not as an
      empirically validated pipeline of this mapping study.
\end{enumerate}
...
the journal's growing body of evidence-based secondary studies while
contributing to the methodological literature on LLM-assisted reviews.

\medskip\noindent\textbf{Reproducibility.}
A complete replication package is openly available at Zenodo
(\href{\zenododoiurl}{DOI~\zenododoi}),
including all search strings, screening prompts, JSON schemas, decision
CSVs at every pipeline stage, the 340-study included list
(\texttt{included\_studies\_340.csv}), the cross-model $\kappa$ scripts,
and the structured extraction tables.
...
\begin{itemize}
\item \textbf{Wil M.\ P.\ van der Aalst} --- RWTH Aachen University,
      foundational PM researcher (\texttt{wvdaalst@pads.rwth-aachen.de})
\item \textbf{Maria Teresa Baldassarre} --- University of Bari, empirical
      SE and secondary studies
