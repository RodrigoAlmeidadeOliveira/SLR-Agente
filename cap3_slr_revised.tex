...
results for the included set. Full QA scoring for all 169 confirmed studies
is in progress; the statistics in this table reflect the subset assessed to date.

\begin{table}[htbp]
\centering
\caption{Quality-assessment results for the confirmed included set (169 total;
  partial QA applied; full QA in progress).}
...
Studies with QA applied          & 76  & Assessed in Round~1 \\
Studies with score $\geq 4/8$   & 71  & Retained for synthesis \\
Studies with score $< 4/8$      & 5   & Excluded after QA \\
Mean QA score (assessed subset) & 5.18 & SD = 1.13 \\
Median QA score                  & 5.00 & IQR (4.00--6.00) \\
Studies pending QA               & 93  & Rounds~2+ (QA not yet applied) \\
\bottomrule
\end{tabular}
\end{table}
...
The broader confirmed set indicates that \textbf{process mining remains the
...
The operational coding of the PDF subset provides consistent quantitative
evidence: \textbf{54 papers} were classified as \textbf{L1}, \textbf{13 as L0},
and only \textbf{6 as L2}. \textbf{No paper} reached \textbf{L3}. Even allowing
for the heuristic nature of the coding, the result is robust enough to support
the central claim that the literature has many partial bridges but no
end-to-end process-aware forecasting architecture for SDLC data.
Figure~\ref{fig:pdf-integration-priority} juxtaposes this pattern with the
reading-priority distribution.
...
Only 52 of 169 confirmed papers (30.8\%) apply stochastic models (IC2/IC4b) to
software processes, and only 18 papers (10.7\%) combine stochastic modeling with
...
further evidenced by EC3 being the second-largest exclusion reason at FT
($n = 197$, 27.5\% of 717 FT excludes): the stochastic modeling community has
not converged on applying its techniques to software development process data.
The full-paper enrichment leads to the same conclusion from a different angle:
among the 73 analyzed PDFs, only 5 show an explicit overlap between process
...
(IC4a $+$ IC4b $+$ IC4c simultaneously) represent only 18 papers (10.7\%) of the
169 confirmed set, and none implements a formally specified end-to-end
architecture. The closest antecedent, PRIMAD \cite{guinea2025}, assembles all
three layers but validates each component independently, leaving the unified
pipeline unevaluated. The 73-paper reading sheets further show that even when
stochastic components are present, validation usually stops at process analysis
or simulation without a downstream learning component.
...
\begin{table}[htbp]
\centering
\caption{Integration levels in the confirmed study set and PATHCAST positioning.}
\label{tab:positioning}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{c p{5.5cm} c p{4.1cm}}
\toprule
\textbf{Level} & \textbf{Description} & \textbf{Papers (PDF subset)}
