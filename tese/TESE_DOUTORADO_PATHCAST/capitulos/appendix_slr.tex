% =============================================================================
% APPENDICES E--I — Systematic Literature Review Supplementary Materials
% Companion to Chapter~\ref{ch:slr}
%
% Usage: include this file AFTER \appendix and appendix_theory (A--D):
%   \appendix
...
% =============================================================================
% APPENDIX E — Search Protocol: Search Strings
% =============================================================================
...
block structure (P and I) is preserved across all adaptations. Strings
S1--S3 correspond to the principal, complementary (MSR), and frontier
(stochastic$\,\wedge\,$PM) blocks, respectively; S4 is a supplementary
string executed in Scopus only, targeting Markov models in software testing.
...
\begin{tabular}{p{2.0cm} c p{9.5cm}}
\toprule
...
Web of Sci.  & S1, S2, S3     & Topic \texttt{TS=(...)}; Article + Proceedings;
                                  English; 1994--2026 \\
...
\section{String S2 --- Complementary (MSR $\rightarrow$ Process Modelling)}
\label{app:slr-s2}
...
\section{String S4 --- Markov in Software Testing (Scopus supplement)}
\label{app:slr-s4}
...
% =============================================================================
% APPENDIX F — LLM Screening Prompts
% =============================================================================
\chapter{LLM Screening Prompts}
\label{app:F}

The review employed the \textit{claude-haiku-4-5-20251001} model as the
primary screener in two phases: title/abstract (T/A) and full text (FT).
Each phase uses a \emph{system prompt} that encodes the scope, criteria, and
output schema, combined with a per-paper \emph{user prompt} that supplies
bibliographic data. All prompts are versioned in
...
The FT-phase system prompt is identical to the T/A prompt, augmented with
three instructions that encode the stricter full-text screening pattern:
...
that supplies the prior decision, rationale, and ICs identified in phase~1:
...
\section{Phase 2b: \textit{Screen-Blanks} Pass}
\label{app:slr-blanks-prompt}

A third pass was applied to the 186 papers without a decision after the two
prior phases (mainly Band~E papers with no abstract available). The
\textit{screen-blanks} pass reuses the same FT system and user prompts, with
the \texttt{abstract} field set to ``Abstract not available'' and the T/A
context block showing \texttt{ta\_decision = maybe} and
\texttt{ta\_matched\_ic = none}. Papers for which the LLM returned
\texttt{pending} and whose title analysis indicated low relevance were closed
under EC5 (full text inaccessible; $n = 148$).

\section{Model Parameters by Phase}
\label{app:slr-prompt-params}

Table~\ref{tab:app-prompt-params} summarises the model parameters used in the
three screening phases and in data extraction.

\begin{table}[htbp]
\centering
\caption{LLM parameters in each screening phase.}
...
Screen-blanks          & claude-haiku-4-5-20251001 & 512  & 186 & Anthropic Batches \\
...
Data extraction for the 169 included studies was performed with the
\textit{claude-haiku-4-5-20251001} model via the Anthropic Batches API.
Papers with a locally available PDF had text pre-extracted with
\texttt{pdfplumber} (first 8 pages, maximum 6{,}000 characters). Papers
without a PDF used the enriched abstract from Semantic Scholar. The prompt
below was submitted once per paper.
...
\{
  ...
  "pm\_technique": "alpha | inductive\_miner | heuristic |
       conformance\_checking | social\_network | declarative |
       other | none",
  ...
  "tool\_used": "tool names (e.g. ProM, pm4py, Disco) or
                not\_mentioned",
  "main\_finding": "main result/contribution (2-3 sentences)",
  "limitations": "reported limitations (1-2 sentences) or
                 not\_mentioned",
...
\section{Data Extraction Form Fields}
\label{app:slr-extraction-fields}

Table~\ref{tab:app-extraction-fields} describes all fields in the structured
...
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{3.4cm} p{4.5cm} p{4.4cm}}
\toprule
...
\texttt{doi}               & Digital identifier               & URI \\
...
\texttt{ft\_screened\_by}  & FT decision responsible          & llm, manual, llm\_pdf \\
\midrule
...
\texttt{pdf\_available}    & PDF available locally            & yes / no \\
\midrule
\multicolumn{3}{l}{\textit{LLM-extracted fields}} \\
\texttt{research\_question}    & Main RQ                      & 1--2 sentences \\
\texttt{study\_type}           & Study type                   & case\_study $|$ experiment $|$ survey $|$ tool $|$ theoretical $|$ simulation $|$ mixed \\
...
\texttt{main\_finding}         & Main result                  & 2--3 sentences \\
\texttt{limitations}           & Study limitations            & 1--2 sentences \\
\texttt{replication\_package}  & Available artefacts          & yes $|$ no $|$ partial $|$ not\_mentioned \\
\midrule
\multicolumn{3}{l}{\textit{Manual fields (reading phase)}} \\
\texttt{quality\_score}    & QA score (0--8)                  & numeric \\
\texttt{extraction\_notes} & Reviewer free-text notes         & free text \\
\bottomrule
\end{tabular}
\end{table}

\section{Controlled Vocabulary (Enums) for Structured Output}
\label{app:slr-schema}

Table~\ref{tab:app-enum-schema} defines the full controlled vocabulary for
structured fields returned by the LLM in both screening phases. These enums
are versioned in \texttt{config/screening\_criteria.py} and were not changed
between screening rounds, ensuring consistency.

\begin{table}[htbp]
\centering
\caption{Controlled vocabulary for LLM structured output.}
...
% =============================================================================
% APPENDIX H — Assessment and Classification Instruments
% =============================================================================
\chapter{Assessment and Classification Instruments}
\label{app:H}

This appendix gathers the instruments used to classify, enrich, and assess
papers throughout screening and selection: the priority-band taxonomy, the
abstract-enrichment algorithm, the quality-assessment checklist, the control
paper set, and the SPMF integration-level taxonomy.

\section{Priority Bands for the Full-Text Queue}
\label{app:slr-bands}

Before full-text screening, the queue of 886 papers was stratified into five
...
\midrule
A & 95  & T/A decision = \texttt{include} and $\geq$2 distinct ICs met \\
B & 20  & T/A decision = \texttt{include} and exactly 1 IC met \\
...
Band~D papers with a DOI were prioritised for manual PDF recovery.
Band~E papers without a DOI or abstract were closed as EC5 (full text
inaccessible) at the end of the \textit{screen-blanks} pass.

\section{Abstract Enrichment Algorithm}
\label{app:slr-enrichment}

To increase the number of papers assessable by the LLM, a four-source
cascade was executed before full-text screening.
Algorithm~\ref{alg:enrichment} describes the enrichment procedure.
...
\hline
\textbf{Input:} Record $p$ with \texttt{doi} and/or \texttt{title} \\
\textbf{Output:} $p$ updated with \texttt{abstract}, \texttt{authors}, \texttt{venue} \\
\hline
...
\quad\quad If found: update \texttt{abstract}, \texttt{authors}, \texttt{venue};
           \textbf{return} \\
\quad\textbf{2b.} Query \textbf{OpenAlex} (\texttt{filter=doi:\{doi\}}) \\
\quad\quad If found: update \texttt{abstract}; \textbf{return} \\
...
\textbf{4.} Set $p$.\texttt{abstract\_source} = \texttt{missing\_or\_unverified} \\
\hline
\end{tabular}
\caption{Abstract enrichment cascade applied to the 886-paper FT queue.}
\label{alg:enrichment}
\end{figure}

The cascade raised abstract coverage from 16.6\% (147/886) to 72.6\%
(643/886) before Round~2 screening. Of the 643 enriched abstracts, 590 were
obtained from Semantic Scholar, 31 from OpenAlex, 13 from CORE, and 9 from
Crossref or the title fallback.
Table~\ref{tab:app-enrichment-stats} summarises results by band.
...
\textbf{Band} & \textbf{Papers} & \textbf{With abstract (pre)} &
  \textbf{Obtained} & \textbf{Coverage (post)} \\
\midrule
...
The eight-item checklist (Table~\ref{tab:app-qa-full}) is applied to each
included primary study during manual reading. Items QA1--QA4 assess
methodological rigour; QA5--QA7 assess empirical robustness and
reproducibility; QA8 specifically checks reporting of process-model quality,
which is central to the PATHCAST evaluation strategy.
Studies scoring below 4/8 are excluded from synthesis.

\begin{table}[htbp]
\centering
\caption{Quality assessment checklist with scoring guide.}
\label{tab:app-qa-full}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{c p{5.5cm} p{5.5cm}}
\toprule
\textbf{ID} & \textbf{Criterion} & \textbf{Scoring guide} \\
\midrule
...
QA2 & Is the software engineering context described in sufficient detail? &
  Score 1 if SDLC phase, artefact type, and process scope are defined. \\
QA3 & Is the data source (event log or repository) described reproducibly? &
  Score 1 if dataset origin, collection method, and key statistics are provided. \\
...
QA5 & Are results validated empirically? &
  Score 1 if validation uses real or semi-real data; 0 for pure simulation or no validation. \\
...
QA7 & Is the study reproducible (data and/or code available)? &
  Score 1 if a replication package or open dataset is described or referenced. \\
...
\midrule
\multicolumn{2}{l}{\textbf{Score range}} & 0--8 (threshold: $\geq$4) \\
\bottomrule
\end{tabular}
\end{table}

Current QA status: 76 of 169 studies assessed (complete Round~1 set);
mean = 5.18 (SD = 1.13), median = 5.0 (IQR: 4--6); 5 studies scored below
the threshold and were excluded from synthesis. The remaining 93 studies
(Rounds~2+) have QA assessment pending.

\section{Control Paper Set}
\label{app:slr-control}

The set of 12 control papers was used to validate search strings before
execution. Table~\ref{tab:app-control} lists all control papers with their
IC profile and the source by which each was captured.

\begin{table}[htbp]
\centering
\caption{Complete control-paper set for string validation.}
\label{tab:app-control}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{c p{4.2cm} p{3.6cm} l l}
\toprule
\textbf{\#} & \textbf{Paper} & \textbf{Rationale} & \textbf{ICs} &
  \textbf{Captured via} \\
\midrule
V1  & Cook \& Wolf (1998)~\cite{cook1998}              & Seminal PM + SE             & IC1, IC2 & ACM \\
V2  & Rubin et al.\ (2007)~\cite{rubin2007}            & Named PM framework for SW (earliest in validation set)   & IC1  & Scopus \\
V3  & Poncin et al.\ (2011)~\cite{poncin2011process}          & PM + MSR intersection         & IC1, IC4 & IEEE \\
...
V7  & Tax et al.\ (2017)~\cite{tax2017}                & Seminal PPM (BPM baseline)  & IC3      & Control \\
...
V10 & Gupta (2017)~\cite{gupta2017}                    & Multi-repository SE PM     & IC1, IC3 & ACM \\
...
\midrule
\multicolumn{4}{l}{Capture rate from primary queries only} & 10/12 (83.3\%) \\
...
recovered via \textit{snowballing} during the FT phase and are included in
the final corpus.
...
\multirow{4}{*}{\textbf{Analytical Depth}}
  & AD1 -- Descriptive   & As-is model discovery; conformance checking \\
...
\multirow{4}{*}{\textbf{Technique Integration}}
  & TI-L0 & Single technique (PM, ML, or stochastic in isolation) \\
  & TI-L1 & Two techniques in the same study without an explicit pipeline contract \\
  & TI-L2 & Partial bridge: two techniques connected, but no ML closure or contract guarantee \\
...
The L0--L3 scale on the TI dimension corresponds to the integration-level
code used in Sections~\ref{sec:slr-rq3} and~\ref{app:slr-top30}.
PATHCAST is positioned at TI-L3, AD4, with SC coverage spanning
SC-D, SC-Q, and SC-C.


% =============================================================================
% APPENDIX I — Reading List and Pipeline Artefacts
% =============================================================================
\chapter{Reading List and Pipeline Artefacts}
\label{app:I}

This appendix presents the ranked list of 32 primary studies selected for
in-depth synthesis and documents the persistent scripts and artefacts of the
review pipeline.

\section{Multi-criterion Scoring Rubric}
\label{app:slr-ranking-criteria}

Table~\ref{tab:app-ranking-criteria} presents the 15-point rubric used to rank
studies. Three seminal papers (Cook 1995, Cook 1998, Rubin 2007) are included
regardless of score.

\begin{table}[htbp]
\centering
\caption{Multi-criterion scoring rubric for the top-32 reading list.}
...
\begin{table}[p]
\centering
\caption{Top-32 reading list with score, year, IC cluster, and summary of the
  main finding. Papers marked with $\dagger$ are forced inclusions due to
  seminal status.}
\label{tab:app-top30}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{c c p{4.0cm} c c p{5.5cm}}
\toprule
...
\midrule
 ...
 2 & 14 & Li et al.\ (2020)~\cite{li2020}
         & IC2,IC3 & \checkmark
         & Hybrid SD+DES simulation for industrial forecasting \\
 ...
 4 & 13 & Ortu et al.\ (2023)~\cite{ortu2023}
         & IC2,IC4 & \checkmark
         & Markov+SNA for fault insertion/correction patterns (Apache) \\
 5 & 13 & Ben Mesmia et al.\ (2021)~\cite{benmesmia2021}
         & IC2,IC3 & \checkmark
         & Non-Markovian SPN for DevOps duration prediction \\
 6 & 12 & Buliga et al.\ (2025)~\cite{buliga2025}
         & IC1,IC3 & \checkmark
         & Generative AI + process simulation for what-if scenarios \\
 7 & 12 & Bhadra et al.\ (2023)~\cite{bhadra2023}
         & IC2,IC3 & \checkmark
         & CI/CD SPN with reliability sensitivity analysis \\
 8 & 12 & Bhadra (2022)~\cite{bhadra2022}
         & IC2,IC3 & \checkmark
         & CI/CD pipeline SPN for delivery probability \\
 9 & 12 & Massitela et al.\ (2018)~\cite{massitela2018}
         & IC2,IC3 & \checkmark
         & Stochastic Automata Network for Waterfall estimation \\
10 & 11 & Incerto et al.\ (2025)~\cite{incerto2025}
         & IC1,IC2 & \checkmark
         & VLMC for stochastic conformance; outperforms 18 techniques \\
11 & 11 & Jo \& Kwon (2024)~\cite{jo2024}
         & IC2,IC4 & \checkmark
         & Probabilistic model checking (CTL) on GitHub repositories \\
12 & 11 & Jo et al.\ (2023)~\cite{jo2023}
         & IC2,IC4 & \checkmark
         & DTMC for GitHub collaboration patterns (5 repos) \\
13 & 11 & L\'opez-Pintado \& Dumas (2023)~\cite{lopezpintado2023}
         & IC1,IC2 & ---
         & Probabilistic resource calendars discovered from logs \\
...
16 & 11 & Lunesu et al.\ (2021)~\cite{lunesu2021}
         & IC2,IC3 & \checkmark
         & Monte Carlo for risk assessment in agile development \\
17 & 11 & Nafreen et al.\ (2020)~\cite{nafreen2020}
         & IC2,IC3 & ---
         & SRGM + defect tracking for hybrid forecasting \\
...
20 & 10 & Jeon et al.\ (2015)~\cite{jeon2015}
         & IC2,IC3,IC4 & ---
         & Probabilistic risk prediction via Markov on repositories \\
21 & 10 & Tian \& Noore (2005)~\cite{tian2005}
         & IC2,IC3 & ---
         & Markov modelling of defect removal under code churn \\
22 & 10 & Cook \& Wolf (1995)$^\dagger$~\cite{cook1995}
         & IC1,IC2 & \checkmark
         & First automated process discovery from traces \\
23 &  9 & Pourbafrani et al.\ (2021)~\cite{pourbafrani2021}
         & IC1,IC3 & \checkmark
         & SIMPT: process tree + what-if simulation tool \\
24 &  9 & Tyagi et al.\ (2021)~\cite{tyagi2021}
         & IC2,IC3 & ---
         & Markov state-space framework for agile planning \\
25 &  9 & Razi et al.\ (2019)
         & IC1,IC2 & \checkmark
         & Conformance checking for SDLC process compliance \\
26 &  9 & Krismayer et al.\ (2019)
         & IC1,IC4 & \checkmark
         & Declarative PM for process monitoring in repositories \\
27 &  9 & Baskoro et al.\ (2019)
         & IC3,IC4 & ---
         & ML-based effort prediction from repositories \\
28 &  9 & (Anon.\ 2017)
         & IC2,IC3 & ---
         & Probabilistic defect prediction in product lines \\
29 &  9 & Magennis (2015)~\cite{magennis2015}
         & IC2,IC3 & ---
         & Economic cycle-time analysis via Monte Carlo \\
30 &  9 & Washizaki et al.\ (2015)~\cite{washizaki2015}
         & IC2,IC3 & ---
         & GSRM release-time prediction for OSS projects \\
31 &  9 & Cook \& Wolf (1998)$^\dagger$~\cite{cook1998}
         & IC1,IC2 & ---
         & Markov/FSA/NN discovery from event-oriented data \\
32 &  2 & Rubin et al.\ (2007)$^\dagger$~\cite{rubin2007}
         & IC1 & \checkmark
         & Named PM framework for SDLC (ProM + SCM logs; earliest in corpus) \\
\bottomrule
\end{tabular}
\end{table}
...
The review was conducted with a custom Python pipeline (\texttt{pipeline/})
operating on structured CSV storage. Table~\ref{tab:app-pipeline} documents
each script with its function and main invocation command.

\begin{table}[htbp]
\centering
\caption{Review pipeline scripts and their functions.}
...
\texttt{pipeline/dedup.py} &
  ...
  \texttt{python main.py dedup} \\
\texttt{pipeline/enrich.py} &
  Abstract enrichment cascade (4 sources) &
  \texttt{python main.py enrich} \\
\texttt{pipeline/screening.py} &
  LLM T/A screening via Batches API &
  \texttt{python main.py screen -{}-run} \\
\texttt{pipeline/fulltext.py} &
  LLM FT screening + manual review &
  \texttt{python main.py fulltext -{}-run} \\
\texttt{pipeline/pdf\_downloader.py} &
  PDF recovery (Unpaywall, S2, OA) &
  \texttt{python main.py pdf} \\
\texttt{pipeline/extract\_prep.py} &
  S2 enrichment + PDF copy for extraction &
  \texttt{python pipeline/extract\_prep.py} \\
\texttt{pipeline/extract\_llm.py} &
  Structured data extraction (LLM, 169 papers) &
  \texttt{python pipeline/extract\_llm.py -{}-run} \\
\texttt{pipeline/synth\_llm.py} &
  Synthesis paragraph generation per IC cluster &
  \texttt{python pipeline/synth\_llm.py} \\
\texttt{pipeline/finalization.py} &
  PRISMA snapshot, EC5 closure, export &
...
Table~\ref{tab:app-artefacts} lists persistent artefacts produced by the review,
with their role, location, and current status.

\begin{table}[htbp]
\centering
\caption{Persistent review artefacts.}
...
\midrule
\texttt{results/screening/ft\_screening\_results.csv}
  & Decision log (886 papers) & Final (no blanks) \\
\texttt{results/extraction/extraction\_template.csv}
  & Extraction data (169 papers) & 169/169 main\_finding \\
\texttt{results/final\_review/included\_studies\_current.csv}
  & Included set & 169 rows \\
\texttt{results/final\_review/top30\_reading\_list.csv}
  & Ranked reading list & 32 rows \\
\texttt{results/final\_review/cap3\_synthesis\_v2.tex}
  & LLM synthesis paragraphs & For review \\
\texttt{results/final\_review/missing\_references.bib}
  & BibTeX for 32 missing keys & Ready to merge \\
\texttt{results/final\_review/pending\_inaccessible\_closed.csv}
  & EC5 closure record & 148 rows \\
\texttt{results/final\_review/prisma\_summary\_current.txt}
  & PRISMA snapshot & Final \\
\texttt{results/SLR\_EXECUCAO\_ATE\_AGORA.md}
  & Execution log (all sessions) & Updated \\
\bottomrule
\end{tabular}
\end{table}
