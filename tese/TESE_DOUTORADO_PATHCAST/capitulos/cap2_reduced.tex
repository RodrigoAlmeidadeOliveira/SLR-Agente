...
{\text{cost}(\gamma_{\text{worst},c})}. \label{eq:case-conformance-cap2}
\end{align}
Thus $\text{conf}(c)$ is the per-case (trace-level) alignment-based fitness:
the cost of the optimal alignment $\gamma^*_c$ of trace $\sigma_c$ normalized
by its worst-case alignment cost, yielding a score in $[0,1]$. Log-level
fitness is the trace-weighted average of $\text{conf}(c)$. The case-level
conformance $\text{conf}(c)$ is reused, under the same definition, as an ML
feature in Chapter~\ref{ch:method}
(Eq.~\ref{eq:case-conformance}).
\end{definition}
...
We refer to this sound, block-structured model $M$ discovered by IMf---the
one that captures the dominant, infrequent-behaviour-filtered control flow
of the SDLC---as the \emph{process backbone}. The term is used throughout the
thesis to denote the discovered control-flow skeleton over which the
stochastic and forecasting layers are subsequently built.
...
(ACE), the quantile-coverage calibration measure of
Kuleshov et al.~\cite{kuleshov2018calibrated},
\begin{equation}
\label{eq:ace}
...
\emph{expected} calibration error (ECE)~\cite{naeini2015obtaining}, which
weights bins by their population, ACE assigns equal weight to each
checkpoint and is therefore
...
derived per case. The on-time/delayed label presupposes a reference target
for each case (a due date, an SLA, or a sprint deadline); the concrete
delay-threshold strategies used to derive this label when no explicit target
is recorded are specified in Chapter~\ref{ch:methodology}. The candidate algorithms (Random
...
yields the discovered \emph{process backbone}---the sound, block-structured
process model (Section~\ref{sec:process-mining}) that captures the dominant control-flow
of the SDLC---and the per-case conformance feature;
