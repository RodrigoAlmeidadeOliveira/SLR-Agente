...
The Markov state space is obtained from the discovered process model by a
one-to-one mapping in which each observable activity $a \in \mathcal{A}$ of
the discovered model becomes a Markov state (state $\equiv$ activity).
Formally, $S = S_T \cup S_A$, where $S_T$ comprises the non-terminal
observed activities and $S_A$ comprises the terminal (absorbing) activities.
By default $S_A = S_{\text{done}} \cup S_{\text{cancelled}}$ with
$S_{\text{done}} = \{\texttt{done}\}$ (successful completion) and
$S_{\text{cancelled}} = \{\texttt{cancelled}\}$; this generalises to
platform-specific terminal states (e.g., \texttt{merged}, \texttt{released}
as completion; \texttt{rejected}, \texttt{duplicate}, \texttt{won't\_fix},
\texttt{archived} as cancellation), which are mapped onto $S_{\text{done}}$
or $S_{\text{cancelled}}$ as appropriate. Transitions of the discovered
model induce the entries of the transition matrix $P$ over $S$.
\end{definition}
