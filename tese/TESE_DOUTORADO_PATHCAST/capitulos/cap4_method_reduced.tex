...
\begin{remark}[Censoring Assumption and Tail Handling]
\label{rem:censoring}
...
repository mining. If the largest observation in $s$ is censored, $\hat{F}_s$
is \emph{defective} (it does not reach $1$); we close it conservatively by
attaching a single exponential tail above the last uncensored time, with rate
matched to the Kaplan--Meier hazard of the final risk set, and report the
fraction of tail mass so closed as a diagnostic. This makes the simulator
proper while keeping the body of $\hat{F}_s$ fully nonparametric. The residual
threat---true sojourns exceeding any observable horizon---is recorded as
...
criterion above. The empirical sojourn distributions $\hat{F}_s$ have finite
support, so all simulated functionals have finite moments and the central
limit theorem applies; the corresponding price is that the empirical
bootstrap cannot extrapolate beyond the largest observed sojourn, so tail
quantiles are conservative lower bounds when the true tail extends past the
observed maximum (a validity threat revisited with assumption~A5).
\end{property}
...
\midrule
A5: Complete case observation   & Downward tail bias if open cases dropped   & Kaplan--Meier sojourn estimator (Def.~\ref{def:km-sojourn}) incorporates right-censored dwells; exponential tail closure for censored maxima \\
\bottomrule
\end{tabular}
\end{table}
