...
(IMf)~\cite{leemans2013discovering} because it is the only discovery
algorithm that guarantees soundness by construction, producing a
...
Dataset~\cite{montgomery2022jira}---reduce candidate-generation cost
without eliminating known validity threats (open-source bias, bot noise,
incompleteness). The construction sub-tasks (extraction, case
correlation, activity mapping) and their archaeological caveats
(stratigraphy, contextual interpretation, taphonomic
bias~\cite{Zimmermann2005MiningVersionHistories}) are operationalised by
...
transitions; PATHCAST mitigates both through optional state-space
expansion and the ML residual signal (Section~\ref{sec:ml-se}).
...
\label{eq:mc-ci}
\end{equation}
The standard error scales as $O(1/\sqrt{M})$ independently of
dimension, and seed control via the Mersenne Twister makes the simulation
...
\end{equation}
CRPS is a strictly proper scoring rule and reduces to mean absolute error
when $\hat{F}$ degenerates to a point mass, so distributional forecasters
and point baselines remain comparable on a common scale. Calibration is
summarised by the aggregate calibration error
\begin{equation}
\label{eq:ace}
\text{ACE} = \tfrac{1}{|\mathcal{A}|}
   \sum_{\alpha \in \mathcal{A}}|\hat{p}_\alpha - \alpha|,
\end{equation}
averaged across decile checkpoints. Worked CRPS-versus-MAE derivations
...
logistic/linear regression baselines, LSTM~\cite{tax2017} for the
...
behavioural heterogeneity and is empirically correlated with lower
...
features alongside MSR features to refine the point predictions; and the
entropy of variants quantifies the process complexity that constrains
forecast confidence. The empirical evidence that this integration has not
been previously proposed for software development is the subject of
Chapter~\ref{ch:slr}.
