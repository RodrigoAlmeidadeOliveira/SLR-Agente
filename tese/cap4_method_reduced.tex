...
$\mathcal{R} = \{r_1, \ldots, r_n\}$, where each repository $r_i$ contains
data sources $r_i = \{D_i^{(1)}, \ldots, D_i^{(k_i)}\}$ with each
$D_i^{(j)}$ a time-ordered sequence of raw events from a specific tool.

\textbf{Output:} A structured event log
$\mathcal{L} = \{\sigma_c \mid c \in \mathcal{C}\}$, where $\mathcal{C}$
is the set of identified cases and each
$\sigma_c = \langle e_1, \ldots, e_{|\sigma_c|} \rangle$ is an ordered
event sequence satisfying Definition~\ref{def:event-log}.
...
\begin{enumerate}
    ...
    \item Activity names belong to a defined SDLC activity taxonomy
    $\mathcal{A}$.
\end{enumerate}
\end{definition}

Stage 1 decomposes into four sub-operations:
\begin{equation}
...
\subsection{Stage 2 Contract}

\begin{definition}[Stage 2 Contract]
\label{def:contract-s2}
\textbf{Input:} Event log $\mathcal{L}$ satisfying
Definition~\ref{def:contract-s1}.

\textbf{Output:} $(M, \Phi, S)$ with discovered model $M$, metrics vector
$\Phi = (\phi_{\text{fit}}, \phi_{\text{prec}}, \phi_{\text{gen}},
\phi_{\text{perf}})$, and state space $S = (S_T, S_A)$.

\textbf{Postconditions:}
\begin{enumerate}
    \item $M$ is sound (no deadlocks, no livelocks).
    \item $\phi_{\text{fit}}, \phi_{\text{prec}} \in [0, 1]$.
    \item $\phi_{\text{perf}}$ includes per-activity sojourn statistics.
    \item $|S_T| \geq 1$ and $|S_A| \geq 1$.
    \item Every $a \in \mathcal{A}$ observed in $\mathcal{L}$ maps to
    exactly one state.
\end{enumerate}
\end{definition}
...
Appendix~G, Section~\ref{app:G-bayes}.
...
state. Proof in Appendix~G, Section~\ref{app:G-fundamental}.
\end{property}
...
Appendix~G, Section~\ref{app:G-semi-markov}.
...
A six-state SDLC chain is fully resolved in Appendix~G,
Section~\ref{app:G-numerical}: matrix computations of $Q, R, N, B$ and
...
\textbf{Input:} $P$, $\hat{F}$, initial state $s_0$ (or distribution
$\pi_0$), and $M_{\text{sim}} \in \mathbb{N}$.
...
\end{equation}
with lead time, terminal state, and trajectory per simulation.
...
\textbf{Output:} Trained predictor $\hat{f}$ and feature importance vector
$\mathbf{w}$.
\textbf{Postconditions:} $\hat{f}$ evaluated on temporally disjoint test
set; AUC-ROC, F1, Brier score, MAE, MAPE reported; $\mathbf{w}$ ranks
features.
\end{definition}
...
The pipeline forms a directed acyclic graph: Stage~2 feeds both Stage~3
and the ML component; Stage~3 feeds both Stage~4 and the ML component.
Inter-stage data contracts are summarized in
Table~\ref{tab:data-contracts}.
...
sequentially on a complete historical dataset.

\textbf{Incremental Forecast (Operational):} given a pre-estimated
$(P, \hat{F}, \hat{f})$ from a prior full run, new cases are scored in
real-time via Stages 4 and ML only. Periodic re-estimation (e.g., weekly).
...
\begin{proof}
By structural induction on stages. Each stage's postconditions imply the
next stage's preconditions: S1 $\to$ S2 (event log validity); S2 $\to$ S3
(sound model + non-empty states); S3 $\to$ S4 (row-stochastic $P$ +
non-empty $\hat{F}$); S4 terminates by absorption certainty
(Property~\ref{prop:absorption-certainty}) or the $K_{\max}$ bound. \qed
\end{proof}
